...
class TestCrocodileAnalyzer:
    ...
    def test_21_conservation_status_function(self, sample_csv_file, capsys):
        """Testa a função function_6_conservation_status"""
        analyzer = CrocodileAnalyzer(sample_csv_file)
        analyzer.function_6_conservation_status()
        
        captured = capsys.readouterr()
        assert "STATUS DE CONSERVAÇÃO" in captured.out
        assert "Least Concern" in captured.out
        assert "Vulnerable" in captured.out
        assert "Critically Endangered" in captured.out
        assert "%" in captured.out


    def test_22_age_class_analysis_function(self, sample_csv_file, capsys):
        """Testa a função function_7_age_class_analysis"""
        analyzer = CrocodileAnalyzer(sample_csv_file)
        analyzer.function_7_age_class_analysis()
        
        captured = capsys.readouterr()
        assert "DISTRIBUIÇÃO POR IDADE" in captured.out
        assert "Adult" in captured.out
        assert "Juvenile" in captured.out
        assert "%" in captured.out


    def test_23_load_data_with_generic_exception(self, tmp_path, capsys):
        """Testa o tratamento de exceção genérica ao carregar dados"""
        # Cria um arquivo CSV inválido/corrompido
        invalid_csv = tmp_path / "invalid.csv"
        invalid_csv.write_text("Invalid,CSV\nContent")
        
        # Mock pd.read_csv para lançar uma exceção genérica
        with patch('pandas.read_csv', side_effect=Exception("Erro genérico")):
            with pytest.raises(SystemExit):
                CrocodileAnalyzer(str(invalid_csv))


    def test_24_basic_info_memory_usage(self, sample_csv_file, capsys):
        """Testa se a função basic_info mostra o uso de memória"""
        analyzer = CrocodileAnalyzer(sample_csv_file)
        analyzer.function_1_basic_info()
        
        captured = capsys.readouterr()
        assert "Tamanho em memória:" in captured.out
        assert "KB" in captured.out
        assert "Tipos de dados:" in captured.out


    def test_25_show_menu_function(self, capsys):
        """Testa a função show_menu que exibe o menu interativo"""
        from crocodile_analyzer_terminal import show_menu
        
        show_menu()
        captured = capsys.readouterr()
        assert "ANÁLISE INTERATIVA DO DATASET DE CROCODILOS" in captured.out or "ANALISADOR DE CROCODILOS" in captured.out
        assert "1.  Informações básicas do dataset" in captured.out or "1.  Informações Básicas" in captured.out
        assert "20. Relatório resumo completo" in captured.out or "20. Relatório Resumo Completo" in captured.out
        assert "0.  Sair" in captured.out


    def test_26_main_function_file_not_found(self, capsys, monkeypatch):
        """Testa a função main quando o arquivo CSV não existe"""
        from crocodile_analyzer_terminal import main
        
        # Simula que o arquivo não existe
        with patch('os.path.exists', return_value=False):
            main()
            captured = capsys.readouterr()
            assert "não encontrado no diretório atual" in captured.out


    def test_27_main_function_exit_option(self, sample_csv_file, capsys, monkeypatch):
        """Testa a função main com opção de saída (0)"""
        from crocodile_analyzer_terminal import main
        
        # Simula entrada do usuário escolhendo a opção 0 (sair)
        monkeypatch.setattr('builtins.input', lambda _: '0')
        
        # Mock para simular que o arquivo existe
        with patch('os.path.exists', return_value=True):
            with patch('crocodile_analyzer_terminal.CrocodileAnalyzer') as mock_analyzer:
                main()
                captured = capsys.readouterr()
                assert "Obrigado por usar" in captured.out or "Digite sua opção" in captured.out


    def test_28_main_function_invalid_input(self, sample_csv_file, capsys, monkeypatch):
        """Testa a função main com entrada inválida seguida de saída"""
        from crocodile_analyzer_terminal import main
        
        # Simula entrada inválida seguida de saída
        # Precisamos de mais valores porque há múltiplas chamadas de input() (escolha + "Pressione ENTER")
        inputs = iter(['abc', '', '0'])  # abc (inválido), ENTER para continuar, 0 (sair)
        ...
        with patch('os.path.exists', return_value=True):
            with patch('crocodile_analyzer_terminal.CrocodileAnalyzer') as mock_analyzer:
                main()
                captured = capsys.readouterr()
                assert "Por favor, digite apenas números" in captured.out or "Digite sua opção" in captured.out


    def test_29_main_function_invalid_option(self, sample_csv_file, capsys, monkeypatch):
        """Testa a função main com opção inválida (fora do intervalo)"""
        from crocodile_analyzer_terminal import main
        
        # Simula entrada de opção inválida seguida de saída
        # Precisamos de mais valores porque há múltiplas chamadas de input() (escolha + "Pressione ENTER")
        inputs = iter(['99', '', '0'])  # 99 (inválido), ENTER para continuar, 0 (sair)
        monkeypatch.setattr('builtins.input', lambda _: next(inputs))
        ...
        with patch('os.path.exists', return_value=True):
            with patch('crocodile_analyzer_terminal.CrocodileAnalyzer') as mock_analyzer:
                main()
                captured = capsys.readouterr()
                assert "Opção inválida" in captured.out or "Digite sua opção" in captured.out


    def test_30_main_function_keyboard_interrupt(self, sample_csv_file, capsys, monkeypatch):
        """Testa a função main com interrupção por teclado (Ctrl+C)"""
        from crocodile_analyzer_terminal import main
        
        # Simula KeyboardInterrupt
        ...
        with patch('os.path.exists', return_value=True):
            with patch('crocodile_analyzer_terminal.CrocodileAnalyzer') as mock_analyzer:
                main()
                captured = capsys.readouterr()
                assert "interrompido pelo usuário" in captured.out or "Digite sua opção" in captured.out
